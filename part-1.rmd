...
plot(auto,ylim=c(0,300))
plot(dauto,ylim=c(-80,80))
...
adf <- adfTest(auto, lag=6, type="ct")
adf


adf <- adfTest_valid(dauto,24,"nc")
adf <- adfTest(dauto, lag=5, type="nc")
adf
...
AICs
AICs==min(AICs)
arima213 <- arima(auto,c(2,1,3),include.mean=F)

BICs
BICs==min(BICs)
arima111 <- arima(auto,c(1,1,1),include.mean=F)
